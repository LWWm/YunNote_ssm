...
<head>
    ...
    <title>云R记</title>
    <base href="http://localhost:8080/YunNote_ssm_war_exploded/">
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
        <div class="container-fluid">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" style="font-size:25px" href="${pageContext.request.contextPath}/index/searchAll">云日记</a>
            </div>
            <!-- Collect the nav links, forms, and other content for toggling -->
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav">
                    <li <c:if test="${menu_page=='index'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/index/searchAll"><i
                            class="glyphicon glyphicon-cloud"></i>&nbsp;主&nbsp;&nbsp;页</a>
                    </li>
                    <li <c:if test="${menu_page=='note'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/note/view"><i
                            class="glyphicon glyphicon-pencil"></i>&nbsp;发布云记</a></li>
                    <li <c:if test="${menu_page=='type'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/type/list"><i
                            class="glyphicon glyphicon-list"></i>&nbsp;类别管理</a></li>
                    <li <c:if test="${menu_page=='user'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/user/userCenter"><i
                            class="glyphicon glyphicon-user"></i>&nbsp;个人中心</a>
                    <li <c:if test="${menu_page=='report'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/report/info"><i
                            class="glyphicon glyphicon-signal"></i>&nbsp;数据报表</a></li>
                </ul>
                <%-- 查询表单 --%>
                <form class="navbar-form navbar-right" role="search" method="post" action="index/searchTitle">
        ...
        <div class="row-fluid">
            <div class="col-md-3">
                <div class="data_list">
                    ...
                    <div class="userimg">
                        <%-- 头像是通过请求后台加载的，所以要请求后台  和 个人中心里加载头像是一样的--%>
                        <img src="${pageContext.request.contextPath}/user/userHead?imageName=${user.head}">
                    </div>
                    ...
                    <div>
                        <ul class="nav nav-pills nav-stacked">
                            <c:forEach items="${dataInfo}" var="item">
                                <li>
                                    <a href="index/searchDate?date=${item.groupName}">${item.groupName}
                        ...
                        <ul id="typeUl" class="nav nav-pills nav-stacked">
                            <c:forEach items="${typeInfo}" var="item">
                                <li id="li_${item.typeId}">
                                    <a href="index/searchType?typeId=${item.typeId}"><span
